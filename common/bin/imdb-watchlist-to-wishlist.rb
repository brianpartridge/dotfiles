...
def ids_not_on_wishlist(watchlist_items, wishlist_items)
  ...
  wishlist_ids = Set.new(wishlist_items.map do |i|
...
def recent_watchlist_items(watchlist)
  ...
  watchlist.items.select { |i| i.date_added > time }.sort_by(&:date_added)
end
...
def run!
  ...
  items_to_add.each do |i|
    puts i
    unless hdbits.add_to_wishlist(i.id)
      puts "ERROR: unable to add #{i.title} to wishlist"
    end
  end
